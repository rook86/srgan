...
for epoch in range(opt.epoch, opt.n_epochs):
    for i, imgs in enumerate(dataloader):
        ...
        for j in range(len(chunks_lr)):
            ...
            loss_GAN = criterion_MSE(fake_pred, valid)
            loss_content = content_loss(generator(chunks_lr[j]), chunks_hr[j])
            loss_pixel = criterion_L1(generator(chunks_lr[j]), chunks_hr[j])
            # Total loss
        ...
        for j in range(len(chunks_lr)):
            ...
            fake_pred = discriminator(generator(chunks_lr[j]))
            loss_real = criterion_MSE(real_pred, valid)
            loss_fake = criterion_MSE(fake_pred, fake)
            # Total loss
